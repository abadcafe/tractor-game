...
Deno.test("test_seat_map_south_is_human", () => {
  assertEquals(SEAT_MAP[2].label, "你");
  assertEquals(SEAT_MAP[2].position, "南");
  assertEquals(SEAT_MAP[2].direction, "south");
  assertEquals(SEAT_MAP[2].team, 0);
});

Deno.test("test_seat_map_north_is_teammate", () => {
  assertEquals(SEAT_MAP[0].label, "同伴");
  assertEquals(SEAT_MAP[0].position, "北");
  assertEquals(SEAT_MAP[0].direction, "north");
...
Deno.test("test_seat_map_west_is_opponent_a", () => {
  ...
  assertEquals(SEAT_MAP[1].direction, "west");
  assertEquals(SEAT_MAP[1].team, 1);
});

Deno.test("test_seat_map_east_is_opponent_b", () => {
  assertEquals(SEAT_MAP[3].label, "右家");
  assertEquals(SEAT_MAP[3].position, "东");
  assertEquals(SEAT_MAP[3].direction, "east");
  assertEquals(SEAT_MAP[3].team, 1);
});

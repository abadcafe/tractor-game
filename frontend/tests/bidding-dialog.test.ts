...
Deno.test("test_renderBiddingDialog_does_not_render_bid_history", () => {
  const snap = makeSnapshot({
    bid_events: [{
      player: 1,
...
Deno.test("test_renderBiddingDialog_stirring_human_has_no_pass_callback", () => {
  const snap = makeSnapshot({
    phase: "STIRRING",
    stirring_state: {
      phase: "WAITING",
  ...
  assertEquals(el.querySelectorAll("button").length, 0);
});

Deno.test("test_renderBiddingDialog_stirring_human_has_no_stir_callback", () => {
  const snap = makeSnapshot({
    phase: "STIRRING",
    stirring_state: {
      phase: "WAITING",
...
Deno.test("test_renderBiddingDialog_clicking_bid_option_calls_onBidOptionSelect", () => {
  ...
  const onBidOptionSelect = (option: BidOption) => {
...
Deno.test("test_renderBiddingDialog_ignores_multiple_bid_events", () => {
  const snap = makeSnapshot({
    bid_events: [
      {
        player: 1,
...
Deno.test("test_renderBiddingDialog_stir_button_not_rendered", () => {
  const snap = makeSnapshot({
    phase: "STIRRING",
    stirring_state: {
      phase: "WAITING",

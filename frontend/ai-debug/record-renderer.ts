import { actionButton, copyButton, textSpan } from "./dom.ts";
import {
  compactJson,
  firstRecord,
  parseJson,
...
export function setTranscriptRecordRerender(
  onChange: () => void,
): void {
  rerender = onChange;
}

export function renderTranscriptRecord(
...
function renderSlot(
  ...
  const isOpen = expanded.has(slotKey)
    ? expanded.get(slotKey) === true
    : false;
  const showRaw = rawViews.has(rawKey)
    ? rawViews.get(rawKey) === true
    : false;
  ...
  kindLabel.textContent = raw === null
    ? "empty"
    : (showRaw ? "raw" : "parsed");
  ...
  if (raw === null || showRaw) {
    ...
    root.appendChild(
      parsed.ok
        ? renderParsedSlot(slot.kind, parsed.value, [
          `record:${record.id}`,
          slot.kind,
        ])
        : preBlock(raw, ""),
...
function apiResponseSummary(value: unknown): string {
  ...
  const tools = message === null
    ? []
    : toolNamesFromCalls(message.tool_calls);
  const toolText = tools.length === 0
    ? "no tool call"
    : (tools.length === 1 ? tools[0] : `${tools.length} tool calls`);
  const usage = body === null ? null : recordValue(body.usage);
  const totalTokens = usage === null
    ? null
    : numericValue(usage.total_tokens);
  return summaryParts([
    status,
    summaryText(choice?.finish_reason) === null
      ? null
      : `finish=${summaryText(choice?.finish_reason)}`,
    toolText,
    totalTokens === null
      ? null
...
function apiErrorSummary(value: unknown): string {
  ...
  const errors = record !== null && Array.isArray(record.errors)
    ? record.errors
    : [value];
  const last = recordValue(errors[errors.length - 1]) ??
    recordValue(value);
...
function toolResultSummary(value: unknown): string {
  ...
  const cardCount = toolCardCount(
    toolCall,
    recordValue(record.message),
  );
  const statusText = status === "rejected" && errorType !== null
    ? `rejected(${errorType})`
    : status;
  if (status === "accepted") {
    return summaryParts([
...
function renderParsedSlot(
  kind: SlotKind,
  value: unknown,
  path: Path,
): HTMLElement {
  if (kind === "api_request") {
    return renderParsedApiRequest(value, path);
  }
  if (kind === "api_response") {
    return renderParsedApiResponse(value, path);
  }
  if (kind === "api_error") return renderParsedApiError(value, path);
  if (kind === "tool_result") {
    return renderParsedToolResult(value, path);
  }
...
function renderParsedApiRequest(
  value: unknown,
  path: Path,
): HTMLElement {
  const root = parsedRoot();
  ...
  root.appendChild(
    sectionBlock("REQUEST SUMMARY", kvGrid(summary), [
      ...path,
      "summary",
    ]),
  );
  if (json !== null) {
    ...
    root.appendChild(
      sectionBlock(
        "TOOLS",
        renderToolList(json.tools, [...path, "tools"]),
        [...path, "tools"],
      ),
    );
  }
...
function renderParsedApiResponse(
  value: unknown,
  path: Path,
): HTMLElement {
  const root = parsedRoot();
  ...
  const summary: KvEntry[] = [
    ...
    [
      "attempt",
      `${textValue(valueAt(value, "attempt"))}/${
        textValue(valueAt(value, "max_attempts"))
      }`,
    ],
  ...
  root.appendChild(
    sectionBlock("RESPONSE SUMMARY", kvGrid(summary), [
      ...path,
      "summary",
    ]),
  );
  if (body === null) {
    root.appendChild(
      sectionBlock(
        "BODY",
        preBlock(stringify(valueAt(value, "body")), "parsed-code"),
        [...path, "body"],
      ),
    );
    return root;
  ...
  if (choice === null) {
    root.appendChild(
      sectionBlock("BODY", preBlock(stringify(body), "parsed-code"), [
        ...path,
        "body",
      ]),
    );
  ...
  if (message === null) {
    root.appendChild(
      sectionBlock(
        "MESSAGE",
        preBlock(stringify(choice), "parsed-code"),
        [...path, "message"],
      ),
    );
    return root;
  ...
  root.appendChild(
    sectionBlock(
      "TOOL CALLS",
      renderToolCalls(message.tool_calls, [...path, "tool_calls"]),
      [
        ...path,
        "tool_calls",
      ],
...
function renderParsedApiError(value: unknown, path: Path): HTMLElement {
  ...
  errors.forEach((error, index) => {
    ...
    list.appendChild(
      sectionBlock(
        title,
        renderObjectSummary(error, [...path, `error:${index}`]),
        [
          ...path,
          `error:${index}`,
        ],
...
function renderParsedToolResult(
  value: unknown,
  path: Path,
): HTMLElement {
  const root = parsedRoot();
  ...
  if (toolCall !== undefined && toolCall !== null) {
    root.appendChild(
      sectionBlock(
        "TOOL CALL",
        renderToolCall(toolCall, [...path, "tool_call"]),
        [...path, "tool_call"],
      ),
    );
  }
  if (message !== undefined && message !== null) {
    root.appendChild(
      sectionBlock(
        "PLAYER MESSAGE",
        renderObjectSummary(message, [...path, "message"]),
        [
          ...path,
          "message",
        ],
  ...
  if (previous !== undefined && previous !== null) {
    root.appendChild(
      sectionBlock(
        ...
        renderObjectSummary(previous, [...path, "previous"]),
        [
          ...path,
          "previous",
        ],
...
function formatApiResponse(value: unknown): string {
  ...
  const lines = [
    ...
    `attempt: ${textValue(valueAt(value, "attempt"))}/${
      textValue(valueAt(value, "max_attempts"))
    }`,
    `duration: ${
      textValue(durationText(valueAt(value, "duration_ms")))
    }`,
...
function toolNamesFromSpecs(tools: unknown): string[] {
  ...
  for (const tool of tools) {
    ...
    const fn = toolRecord === null
      ? null
      : recordValue(toolRecord.function);
...
function toolNamesFromCalls(toolCalls: unknown): string[] {
  ...
  for (const call of toolCalls) {
    ...
    const fn = callRecord === null
      ? null
      : recordValue(callRecord.function);
    const name = summaryText(fn?.name);
    if (name !== null) result.push(name);
  }
  return result;
...
function toolCardCount(
  toolCall: Record<string, unknown> | null,
  message: Record<string, unknown> | null,
): number | null {
  const argumentsRecord = toolCall === null
    ? null
    : recordValue(toolCall.arguments);
...
function renderToolList(tools: unknown, path: Path): HTMLElement {
  ...
  tools.forEach((tool, index) => {
    ...
    list.appendChild(
      sectionBlock(`TOOL ${index + 1}`, body, [
        ...path,
        `tool:${index}`,
      ]),
    );
...
function renderToolCall(value: unknown, path: Path): HTMLElement {
  ...
  root.appendChild(sectionBlock(
    ...
    preBlock(
      argumentsParsed.ok
        ? stringify(argumentsParsed.value)
        : argumentsRaw,
      "parsed-code",
    ),
    [...path, "arguments"],
...
function renderObjectSummary(value: unknown, path: Path): HTMLElement {
  ...
  if (simpleEntries.length > 0) {
    root.appendChild(
      sectionBlock("SUMMARY", kvGrid(simpleEntries), [
        ...path,
        "summary",
      ]),
    );

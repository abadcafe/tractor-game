...
`is_joker`、`is_big_joker`、`deck` 不进入协议；这些都是可以由
`id/suit/rank` 推导出来的内部属性。
...
非空 `action_hints` 可以被 UI/AI 当作封闭候选集使用。 因此 server
不能只返回前几个 hint；如果无法完整返回，就必须返回 `[]`。

抢主 hint 是“逻辑动作”的完整集合，牌 ID 只用作 canonical 表示。
例如手里两张等价的黑桃级牌时，单张黑桃抢主只会用其中一个 card id
表示这一种逻辑抢主动作，而不是枚举两个等价 card id。

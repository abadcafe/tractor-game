...
class AIPlayer(Player):
    ...
    def __init__(
        ...
        *,
        ...
        self._transcript_records_by_seq: dict[int, TranscriptRecord] = {}
    ...
    async def on_state(self, game: GameView, message: StateMessage) -> None:
        """Update visible memory and schedule one decision if action is awaited."""
        ...
        if message.error is not None:
            ...
            if self._should_repair_server_rejection(message, rejected_record):
        ...
        for attempt in range(start_attempt, max_attempts + 1):
            ...
            if isinstance(decision_result_from_client, Rejected):
                record = self._transcript.add_record(
                    ...
                    seq=message.seq,
                    attempt=attempt,
                    api_request=_api_from_rejection(decision_result_from_client).request,
                    api_response=_api_from_rejection(decision_result_from_client).response,
                    api_error=_client_rejection_api_error(decision_result_from_client),
                    tool_result=None,
                )
            ...
            decision_result = tool_call_to_message(message.seq, snapshot, decision.tool_call)
            if isinstance(decision_result, Ok):
                record = self._transcript.add_record(
                    ...
                    tool_result=_accepted_tool_result(decision.tool_call, decision_result.value),
                )
                self._transcript_records_by_seq[message.seq] = record
    ...
    def subscribe_transcript(self) -> asyncio.Queue[TranscriptRecordDict]:
        return self._transcript.subscribe()

    def unsubscribe_transcript(self, queue: asyncio.Queue[TranscriptRecordDict]) -> None:
        self._transcript.unsubscribe(queue)

    def _record_server_rejection(self, message: StateMessage) -> TranscriptRecord:
        ...
        if record is None:
            record = self._transcript.add_record(
                ...
                api_response=None,
                api_error=None,
                tool_result=_server_rejection_tool_result(message.error, previous=None),
            )
            self._transcript_records_by_seq[message.seq] = record
            return record
        self._transcript.update_tool_result(
            record,
            _server_rejection_tool_result(message.error, previous=record.tool_result),
        )
        return record
...
def _client_rejection_api_error(rejection: Rejected) -> str:
    ...
    if isinstance(rejection, AIClientRejected) and api.error is not None:
        return api.error
    return _json_text({

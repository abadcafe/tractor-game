...
def build_decision_prompt(
    *,
    ...
    system = "\n\n".join(
        [
            "你是升级/拖拉机游戏里的 AI 玩家。",
            "你只能根据当前 player 可见的信息决策，"
            "不能假设其他玩家手牌。",
            "你必须且只能调用一个当前允许的动作 tool。",
            "tool 参数里的 card_ids 只能从“你的手牌”或 "
            "action_hints 中逐字复制。",
            "如果当前 action_hints 非空，card_ids 必须完整等于"
            "其中一个 hint 组，不能只取"
            "hint 组的一部分。",
...
def _state_summary(player_index: int, snapshot: StateSnapshot) -> str:
    trump_suit = (
        snapshot.trump_suit
        if snapshot.trump_suit is not None
        else "no_trump"
    )
    return "\n".join(
        [
...
def _hand_summary(hand: list[Card]) -> str:
    ...
    for card in hand:
        lines.append(
            f"- {card.id}: {card_text(card)},points={card_points(card)}"
        )
    return "\n".join(lines)
...
def _trick_summary(trick: TrickSnapshot | None) -> str:
    ...
    for slot in trick.slots:
        ...
        played_cards = cards if cards else "not_played"
        lines.append(f"- player {slot.player}: {played_cards}")
    return "\n".join(lines)

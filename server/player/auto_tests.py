...
async def test_auto_player_play_when_current() -> None:
    ...
    test_card = card(Suit.SPADES, Rank.ACE, 1)
    snap = make_snapshot(
        phase="PLAYING",
...
async def test_auto_player_play_from_action_hints() -> None:
    ...
    card1 = card(Suit.SPADES, Rank.ACE, 1)
    snap = make_snapshot(
        phase="PLAYING",
        awaiting_action="play",
        player_hand=[card1],
...
async def test_auto_player_discard_when_current() -> None:
    ...
    card1 = card(Suit.DIAMONDS, Rank.THREE, 1)
    card2 = card(Suit.CLUBS, Rank.FOUR, 1)
    snap = make_snapshot(
        phase="STIRRING",
...
async def test_auto_player_stir_when_current() -> None:
    ...
    card1 = card(Suit.HEARTS, Rank.TWO, 1)
    card2 = card(Suit.HEARTS, Rank.TWO, 2)
    snap = make_snapshot(
        phase="STIRRING",
...
async def test_auto_player_bid_during_dealing() -> None:
    """AutoPlayer can bid during DEAL_BID phase."""
    trump_card = card(Suit.HEARTS, Rank.TWO, 1)
    snap = make_snapshot(
        phase="DEAL_BID",
        awaiting_action="bid",
...
async def test_auto_player_ignores_dealing_if_no_trump_rank() -> None:
    ...
    non_trump = card(Suit.SPADES, Rank.THREE, 1)
    snap = make_snapshot(
        phase="DEAL_BID",
        awaiting_action="bid",
...
async def test_auto_player_stir_only_uses_same_suit_pairs() -> None:
    ...
    card_hearts_2_d1 = card(Suit.HEARTS, Rank.TWO, 1)
    card_spades_2_d1 = card(Suit.SPADES, Rank.TWO, 1)
    card_hearts_2_d2 = card(Suit.HEARTS, Rank.TWO, 2)
...
async def test_auto_player_discard_with_stirring_exchange_count() -> None:
    ...
    card1 = card(Suit.DIAMONDS, Rank.THREE, 1)
    card2 = card(Suit.CLUBS, Rank.FOUR, 1)
    card3 = card(Suit.SPADES, Rank.FIVE, 1)

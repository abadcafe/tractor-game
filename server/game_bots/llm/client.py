"""Provider-neutral client contracts for LLM decisions."""
...
class DecisionPrompt:
    """Prompt payload for one LLM decision."""
...
@dataclass(frozen=True, slots=True)
class ClientRejected(Rejected):
    """LLM client rejection with raw API interaction details."""

    api: APIInteraction = field(default_factory=APIInteraction)


class DecisionClientDisabledRejected(Rejected):
    reason_text: ClassVar[str] = "LLM decision client is disabled"
...
class OpenAIAPIKeyMissingRejected(Rejected):
    reason_text: ClassVar[str] = "TRACTOR_LLM_API_KEY is not set"
...
class OpenToolCallsMissingRejected(Rejected):
    reason_text: ClassVar[str] = (
...
class OpenToolCallCountRejected(Rejected):
...
class OpenToolCallNotObjectRejected(Rejected):
    reason_text: ClassVar[str] = (
...
class OpenToolCallNotFunctionRejected(Rejected):
    reason_text: ClassVar[str] = (
...
class OpenToolCallMissingFunctionRejected(Rejected):
    reason_text: ClassVar[str] = (
...
class DecisionClient(Protocol):
    """Provider-neutral interface used by LLMPolicy."""
...
class DisabledDecisionClient:
    ...
    async def decide(
        ...
        return DecisionClientDisabledRejected()

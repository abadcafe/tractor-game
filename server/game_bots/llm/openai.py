"""OpenAI-compatible Chat Completions client for LLM decisions."""
...
from server.game_bots.llm.client import (
    APIInteraction,
    ClientRejected,
    Decision,
    DecisionClient,
    ...
    OpenToolCallCountRejected,
    OpenToolCallMissingFunctionRejected,
    OpenToolCallNotFunctionRejected,
    OpenToolCallNotObjectRejected,
    OpenToolCallsMissingRejected,
    ToolCall,
    ToolSpec,
    is_json_object,
    is_json_value,
)
from server.game_bots.llm.config import LLMConfig
...
@dataclass(frozen=True, slots=True)
class OpenAIChatCompletionsClient(DecisionClient):
    """
    ...
    async def _post(
        ...
        endpoint: str,
        payload: JSONObject,
    ) -> Ok[_OpenAIHTTPResponse] | ClientRejected:
    ...
    async def _post_once(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "LLM request error: attempt=%d/%derror=%s",
                attempt,
                max_attempts,
                exc,
            )
        ...
        try:
            ...
            logger.warning(
                "LLM HTTP error: attempt=%d/%dstatus=%s body=%s",
                attempt,
                max_attempts,
                status_code,
        ...
        logger.debug(
            "LLM HTTP success: attempt=%d/%d status=%s",
            attempt,
            max_attempts,
        ...
        try:
            ...
            logger.warning(
                "LLM provider returned invalid JSON: %s",
...
def _tool_to_json(tool: ToolSpec) -> JSONObject:
    return {
        "type": "function",
        "function": {
            "name": tool.name,
...
def extract_chat_completion_tool_call(
    ...
    if not _is_object_list(tool_calls):
        ...
        return OpenToolCallsMissingRejected()
    calls: list[ToolCall] = []
    ...
    if len(calls) != 1:
        return OpenToolCallCountRejected(len(calls))
    return Ok(calls[0])
...
def _extract_one_tool_call(item: object) -> Ok[ToolCall] | Rejected:
    if not _is_object_dict(item):
        return OpenToolCallNotObjectRejected()
    if item.get("type") != "function":
        return OpenToolCallNotFunctionRejected()
    function = item.get("function")
    if not _is_object_dict(function):
        return OpenToolCallMissingFunctionRejected()
    ...
    return Ok(ToolCall(name=name, arguments=arguments_obj))
...
def _api_with_error(api: APIInteraction, error: str) -> APIInteraction:

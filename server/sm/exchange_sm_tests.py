...
def _make_hand(count: int, offset: int = 0) -> list[Card]:
    """
    Create a hand of `count` cards with unique IDs, starting from
    `offset`.
    """
    cards: list[Card] = []
...
class TestDiscard:
    def test_discard_correct_count_transitions_to_complete(
        self,
    ) -> None:
        """
        Discard exactly count cards -> state transitions to COMPLETE
        phase.
        """
        hand = _make_hand(25)
        bottom = _make_hand(8, offset=25)
...
class TestDiscardResult:
    ...
    def test_exchange_result_on_state(self) -> None:
        ...
        assert (
            len(new_state.result.new_hand)
            + len(new_state.result.new_bottom_cards)
            == 33
        )

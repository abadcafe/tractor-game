...
def _make_input(
    *,
    ...
    if players_hand is None:
        ...
        for i in range(4):
            ...
            for rank in [
                ...
                cards.append(
                    _card(
                        Suit.HEARTS if i == 0 else Suit.SPADES, rank, 1
                    )
                )
                cards.append(
                    _card(
                        Suit.HEARTS if i == 0 else Suit.SPADES, rank, 2
                    )
                )
    ...
    return StirInput(
        ...
        initial_bid_cards=[]
        if initial_bid_cards is None
        else initial_bid_cards,
...
def _complete_initial_exchange(state: StirringState) -> StirringState:
    """
    Complete the initial EXCHANGING sub-phase by discarding bottom
    cards.
    """
    assert state.phase == "EXCHANGING"
    assert state.exchange_state is not None
    assert state.exchanging_player is not None
    discards = state.exchange_state.hand_after_pickup[
        : state.exchange_state.count
    ]
    result = stir_discard(
        state, player=state.exchanging_player, cards=discards
    )
...
class TestCreateStirring:
    ...
    def test_create_stirring_single_initial_bid_uses_single_priority(
        self,
    ) -> None:
        """
        ...
        result = stir(
            state, player=state.current_player, cards=players_hand[3]
        )

        assert isinstance(result, Ok)
        assert result.value.trump_suit == Suit.DIAMONDS

    def test_create_stirring_joker_initial_bid_uses_joker_priority(
        self,
    ) -> None:
...
class TestStirDiscard:
    def test_stir_discard_completes_initial_exchange(self) -> None:
        """
        After stir_discard, phase transitions to WAITING and skips
        exchanger.
        """
        state = create_stirring(
            _make_input(
    ...
    def test_stir_discard_wrong_player_rejected(self) -> None:
        ...
        discards = (
            state.exchange_state.hand_after_pickup[:8]
            if state.exchange_state
            else []
        )
    ...
    def test_stir_discard_not_in_exchanging_rejected(self) -> None:
        ...
        state = _complete_initial_exchange(
            create_stirring(
                _make_input(
                    trump_suit=Suit.HEARTS,
                    trump_rank=Rank.TWO,
    ...
    def test_stir_discard_updates_hands_and_bottom(self) -> None:
        ...
        assert isinstance(result, Ok)
        new_state = result.value
        # Player 0's hand should be updated (original + bottom -
        # discards)
        assert (
            len(new_state.players_hand[0]) == 1
        )  # 1 original + 1 bottom - 1 discard
...
class TestPassStir:
    ...
    def test_pass_stir_all_pass_complete(self) -> None:
        """
        When the three non-exchanging players pass, phase becomes
        COMPLETE.
        """
        state = _complete_initial_exchange(
            create_stirring(
...
class TestStir:
    ...
    def test_stir_empty_trump_diamond_pair_accepted(self) -> None:
        """
        空主: even ♦ pair can stir.

        Lowest priority still beats no trump.
        """
        state = _complete_initial_exchange(
            create_stirring(
                _make_input(
    ...
    def test_stir_empty_trump_small_joker_then_big_joker(self) -> None:
        """
        空主: small joker pair → big joker pair accepted (higher
        priority).
        """
        state = _complete_initial_exchange(
            create_stirring(
        ...
        result = stir(
            state, player=state.current_player, cards=small_jokers
        )
        assert isinstance(result, Ok)
        assert result.value.trump_suit is None
        ...
        result = stir(
            state, player=state.current_player, cards=big_jokers
        )
        assert isinstance(result, Ok)
        assert result.value.trump_suit is None
        assert result.value.current_priority == 205

    def test_stir_empty_trump_big_joker_then_small_joker_rejected(
        self,
    ) -> None:
        """
        空主: after big joker pair, phase is COMPLETE (no higher stir
        possible).
        """
        state = _complete_initial_exchange(
            create_stirring(
    ...
    def test_stir_empty_trump_joker_no_infinite_loop(self) -> None:
        ...
        state = _complete_initial_exchange(result.value)
        assert state.current_priority == 205
        # Big joker pair is max priority → COMPLETE directly, no pass
        # needed
        assert state.phase == "COMPLETE"

    def test_stir_big_joker_pair_skips_waiting(self) -> None:
        """
        Big joker pair (max priority) skips WAITING, goes directly to
    ...
    def test_stir_small_joker_pair_goes_to_waiting(self) -> None:
        """
        Small joker pair (not max) goes to WAITING — others could still
        stir.
        """
        state = _complete_initial_exchange(
            create_stirring(
        ...
        assert result.value.phase == "EXCHANGING"
        assert result.value.current_priority == 204
        # Complete the exchange — should go to WAITING (big joker could
        # still beat it)
    ...
    def test_last_stir_player_is_skipped_after_exchange(self) -> None:
        """
        Player who just stirred is not asked again in the same stir
        cycle.
        """
        state = _complete_initial_exchange(
            create_stirring(
        ...
        # Other three players pass; the turn never returns to the
        # stirrer.
        for _ in range(3):
            result = pass_stir(state, player=state.current_player)
            assert isinstance(result, Ok)
...
class TestStirFullFlow:
    def test_stir_full_flow_multiple_stirs(self) -> None:
        ...
        result = stir(
            state, player=state.current_player, cards=cards_club
        )
        assert isinstance(result, Ok)
        state = _complete_initial_exchange(result.value)
        ...
        result = stir(
            state, player=state.current_player, cards=cards_spade
        )
        assert isinstance(result, Ok)
        state = _complete_initial_exchange(result.value)
    ...
    def test_stir_discard_after_stir_updates_hands(self) -> None:
        ...
        result2 = stir_discard(
            result.value,
            player=result.value.exchanging_player,
            cards=discards,
        )

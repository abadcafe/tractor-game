"""Compile legal action spaces into queue-safe action specs."""
...
def compile_legal_action_spec(
    legal_action: LegalActionSpace,
) -> CompiledActionSpec:
    """Compile one legal action space into an internal data spec."""
    if isinstance(legal_action, EmptyLegalActionSpace):
        return CompiledActionSpec(
            kind="empty", trace_set=None, selection=None
        )
    if isinstance(legal_action, CompleteTraceLegalActionSpace):
        return _compile_trace_set(legal_action)
    if isinstance(legal_action, DiscardLegalActionSpace):
    ...
    if isinstance(legal_action, LeadPlayLegalActionSpace):
        query = legal_action.query
        return CompiledActionSpec(
            kind="lead_play",
            trace_set=None,
    ...
    if isinstance(legal_action, FollowPlayLegalActionSpace):
        query = legal_action.query
    ...
    raise AssertionError(
        f"unknown legal action space: {legal_action!r}"
    )


def _compile_trace_set(
    legal_action: CompleteTraceLegalActionSpace,
) -> CompiledActionSpec:
    return CompiledActionSpec(
        kind="trace_set",

...
class ProcessInspector:
    ...
    def inspect_pid(
        ...
        initial_stat_result = _parse_stat(
            initial_stat_text, process_dir, expected_pid=pid
        )
        if isinstance(initial_stat_result, _result.Rejected):
            return initial_stat_result
        final_stat_result = _parse_stat(
            final_stat_text, process_dir, expected_pid=pid
        )
        if isinstance(final_stat_result, _result.Rejected):
            return final_stat_result
        if initial_stat_result.value[3] != final_stat_result.value[3]:
            return _result.Rejected(
                reason=(

...
from server.game.rules.rejections.play import MustFollowLeadSuitRejected
...
class TestPlayLead:
    ...
    def test_play_failed_throw_forces_smallest_failed_subplay(
        self,
    ) -> None:
        """
        Failed throw advances state, exposes attempted cards, and plays
        forced cards.
        """
        hands = [
            [
...
class TestPlayResolve:
    ...
    def test_play_resolve_determines_winner(self) -> None:
        """
        Winner is determined by compare_plays: highest same-suit card
        wins.
        """
        hands = [
...
class TestPlayValidation:
    ...
    def test_play_follow_must_follow_suit(self) -> None:
        """
        Following player must play cards of the led effective suit if
        they have any.
        """
        ...
        assert (
            result.reason
            == "必须跟首出花色：首出是红桃，你手里还有红桃。"
        )
        # ♠Q -- illegal
        # Player 1 plays ♥K (correct follow-suit) -- should succeed
        result = play(
            state, player=1, cards=[hands[1][0]]
        )  # ♥K -- legal
        assert isinstance(result, Ok)
        state = result.value
        assert state.played == 2
    ...
    def test_play_follow_no_suit_can_play_anything(self) -> None:
        """
        Following player with no cards of the led suit can play any
        card.
        """
...
class TestPlayFollowPairSuit:
    def test_play_follow_pair_must_follow_suit(self) -> None:
        """
        Following a pair lead: must play pair of the led suit if
        possible.
        """
        ...
        assert isinstance(result, Ok)
        state = result.value
        # Player 1 tries to play ♠Q pair (off-suit) -- should be
        # rejected
    ...
    def test_play_follow_no_pair_can_play_anything(self) -> None:
        """
        Following a pair lead: with no pair of led suit, can play any
        pair.
        """
...
class TestPlayFollowTractorSuit:
    def test_play_follow_tractor_must_follow_suit(self) -> None:
        """
        Following a tractor lead: must play matching-length tractor of
        same suit if possible.
        """
        # Lead: tractor of ♥A pair + ♥K pair (4 cards).
        # Player 1 has both ♥Q pair + ♥J pair (matching tractor) and ♠Q
        # pair + ♠J pair.
        ...
        hands = [
            ...
            [
                ...
                _card(Suit.SPADES, Rank.QUEEN, 1),
                _card(Suit.SPADES, Rank.QUEEN, 2),
                _card(Suit.SPADES, Rank.JACK, 1),
                _card(Suit.SPADES, Rank.JACK, 2),
            ],
            [
        ...
        assert isinstance(result, Ok)
        state = result.value
        # Player 1 tries to play ♠Q+♠J pair (off-suit tractor) -- should
        # be rejected
        off_suit = hands[1][4:8]
        result = play(state, player=1, cards=off_suit)
        assert isinstance(result, Rejected)
        # Player 1 plays ♥Q+♥J pair (correct follow-suit tractor) --
        # should succeed
        on_suit = hands[1][:4]
...
class TestPlayResolveNewComparison:
    def test_no_trump_level_cards_tie_so_leader_wins(self) -> None:
        """
        No-trump level cards in different suits are equal; first played
        wins.
        """
        hands = [
    ...
    def test_off_suit_level_cards_tie_so_earlier_player_wins(
        self,
    ) -> None:
    ...
    def test_resolve_equal_cards_keeps_earlier_play_order_winner(
        self,
    ) -> None:
        """
...
def _play_unwrap(
    state: TrickState, player: int, cards: list[Card]
) -> TrickState:
...
class TestTractorRanking:
    ...
    def test_resolve_trump_tractor_beats_non_trump_tractor(
        self,
    ) -> None:

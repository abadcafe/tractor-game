...
class Card(BaseModel):
    ...
    def _validate_card_consistency(self) -> Self:
        ...
        if self.points != card_points(self.rank):
            raise ValueError("points must equal card_points(rank)")
        return self
...
def card_points(rank: Rank) -> int:
    """Return the immutable point value of a rank."""
    return _POINTS[rank]


def suited_ranks() -> tuple[Rank, ...]:
    """Return ranks carried by ordinary suited cards."""
    return _SUITED_RANKS

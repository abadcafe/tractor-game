"""Black-box tests for follow-play legality."""

from typing import Literal

from server.foundation.result import Ok
from server.game.rules import play
from server.game.rules.cards import Card, Rank, Suit, card_points
...
def is_legal_lead(
    ...
    return isinstance(
        play.resolve_lead(
            ...
            trump_suit,
            trump_rank,
            tuple(tuple(cards) for cards in other_hands),
        ),
        Ok,
    )
...
class TestIsLegalLead:
    def test_is_legal_lead_single(self) -> None:
        ...
        assert (
            is_legal_lead(hand, played, Suit.SPADES, Rank.TWO, [])
            is True
        )
    ...
    def test_is_legal_lead_tractor(self) -> None:
        ...
        assert (
            is_legal_lead(hand, hand, Suit.SPADES, Rank.TWO, []) is True
        )

    def test_is_legal_lead_throw_valid(self) -> None:
        """Throw with all biggest sub-plays is legal.

        spA spK with no other sp cards in other hands -> both are
        biggest singles.
        """
        hand = [
        ...
        assert (
            is_legal_lead(
                hand, hand, Suit.HEARTS, Rank.TWO, [other_hands]
            )
            is True
        )

    def test_is_legal_lead_throw_failed_single_still_submittable(
        self,
    ) -> None:
        ...
        result = is_legal_lead(
            hand, hand, Suit.HEARTS, Rank.TWO, [other_hands]
        )
        assert result is True

    def test_is_legal_lead_throw_pair_not_biggest_still_submittable(
        self,
    ) -> None:
        """
    ...
    def test_is_legal_lead_different_suits(self) -> None:
        ...
        played = hand[:]
        assert (
            is_legal_lead(hand, played, Suit.DIAMONDS, Rank.TWO, [])
            is False
        )

    def test_is_legal_lead_trump_and_non_trump_mix(self) -> None:
        """
        Mixing trump and non-trump cards -> different effective suits ->
        illegal.
        """
    ...
    def test_is_legal_lead_throw_tractor_not_biggest_still_submittable(
        self,
    ) -> None:
        """
        A throw containing a non-biggest tractor is still submittable.
        """
        # Hand: tractor sp3-3-4-4 + single spA. Other hand has tractor
        # sp5-5-6-6.
        hand = [
    ...
    def test_is_legal_lead_throw_biggest_tractor(self) -> None:
        ...
        # Hand: tractor spK-K-A-A + single spQ. No bigger sp tractors or
        # singles in others.
        hand = [
            _card(Suit.SPADES, Rank.KING, 1),
            _card(Suit.SPADES, Rank.KING, 2),
...
class TestIsLegalFollow:
    ...
    def test_is_legal_follow_wrong_count(self) -> None:
        ...
        assert (
            is_legal_follow(hand, played, lead, Suit.SPADES, Rank.TWO)
            is False
        )
    ...
    def test_is_legal_follow_no_trump_rank_card_is_not_lead_suit(
        self,
    ) -> None:
        """
        In no-trump, rank cards are trump and do not satisfy their
        printed suit.
        """
        hand = [
        ...
        assert (
            is_legal_follow(hand, played, lead, None, Rank.FOUR) is True
        )
    ...
    def test_is_legal_follow_pair_no_pair_play_two_singles(
        self,
    ) -> None:
        """
        No pair of lead suit -> must play 2 cards of lead suit if
        available.
        """
        hand = [
            _card(Suit.HEARTS, Rank.ACE),
    ...
    def test_is_legal_follow_pair_no_suit_play_any_two(self) -> None:
        ...
        assert (
            is_legal_follow(hand, hand, lead, Suit.SPADES, Rank.TWO)
            is True
        )

    # --- Tractor following ---
    def test_is_legal_follow_tractor_must_play_matching_tractor(
        self,
    ) -> None:
    ...
    def test_is_legal_follow_tractor_priority_from_high_to_low(
        self,
    ) -> None:
        """Must use higher-level sub-plays first when following tractor.

        Lead: 2-pair tractor (4 cards). Hand has 3-pair tractor +
        independent pair.
        Must use the 3-pair tractor (take 2 pairs from it), not the
        independent pair.
        """
        ...
        assert (
            is_legal_follow(
                hand, legal_play, lead, Suit.SPADES, Rank.TWO
            )
            is True
        )
        ...
        assert (
            is_legal_follow(
                hand, illegal_play, lead, Suit.SPADES, Rank.TWO
            )
            is False
        )
    ...
    def test_is_legal_follow_tractor_partial_with_singles(self) -> None:
        """
        No matching tractor, have pairs -> play all pairs + fill with
        singles.
        """
        hand = [
            _card(Suit.HEARTS, Rank.ACE, 1),
    ...
    def test_is_legal_follow_trump_as_lead_eff(self) -> None:
        """
        Trump cards have effective suit 'trump'. Following trump lead
        must play trump.
        """
    ...
    def test_is_legal_follow_tractor_non_contiguous_extraction(
        self,
    ) -> None:
        """Partial extraction from a tractor must be contiguous.

        Hand has a 3-pair tractor h3-3-4-4-5-5. Lead is a 2-pair
        tractor.
        Playing h3-3 + h5-5 (skipping h4-4) should be illegal because
        the extracted pairs are not contiguous in the tractor's rank
        sequence.
        """
        hand = [
            _card(Suit.HEARTS, Rank.THREE, 1),
    ...
    def test_is_legal_follow_fewer_suit_cards_tractor(self) -> None:
        ...
        Lead is a 4-card tractor (2 pairs). Hand has 1 pair + 1 single
        of
        lead suit (3 cards) + 2 non-suit cards. Must play all 3 suit
        cards
        ...
        # Should fail: hand has 3 hearts (hA-A pair + hK single) but
        # only 2 hearts played
        assert (
            is_legal_follow(
                hand, illegal_play, lead, Suit.SPADES, Rank.TWO
            )
    ...
    def test_is_legal_follow_fewer_suit_cards_throw(self) -> None:
        ...
        Lead is a 3-card throw. Hand has 2 cards of lead suit + 1
        non-suit.
        ...
        # Illegal: skip a heart (play 1 heart + 2 non-hearts -- but only
        # 1 non-heart)
        # With only 1 non-heart, can't make 3 cards skipping a heart.
        # Count would be wrong.
        # So let's test: hand has 2 hearts + 2 spades. Play 1 heart + 2
        # spades.
        hand2 = [
            _card(Suit.HEARTS, Rank.ACE),
            _card(Suit.HEARTS, Rank.KING),
        ...
        assert (
            is_legal_follow(
                hand2, illegal_play, lead, Suit.SPADES, Rank.TWO
            )
            is False
        )
    ...
    def test_is_legal_follow_no_pairs_in_hand_tractor(self) -> None:
        """
        No pairs at all in lead suit when following a tractor: play any
        N cards.
        """
        lead = [
            _card(Suit.HEARTS, Rank.THREE, 1),
        ...
        # No pairs in hand, all singles. Must play 4 cards but only 3
        # hearts.
        # With fewer suit cards: must play all 3 hearts + 1 fill
        # But we have no fill cards. So this should be... actually count
        # must match.
        # Lead is 4 cards, played must be 4. Hand has 3 hearts, 0
        # others.
        # Can't make 4 cards. So this scenario can't happen with only 3
        # cards.
        ...
        # 4 hearts, no pairs. Lead is 4-card tractor. Must play all 4
        # hearts.
        played = [
            _card(Suit.HEARTS, Rank.ACE),
            _card(Suit.HEARTS, Rank.KING),
        ...
        assert (
            is_legal_follow(hand2, played, lead, Suit.SPADES, Rank.TWO)
            is True
        )

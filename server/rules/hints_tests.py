...
from server.rules.hints import get_legal_play_hints, sort_play_action_hints, TOO_MANY_PLAY_HINTS
...
class TestGetLegalPlayHints:
    def test_no_lead_cards_returns_no_hints(self) -> None:
        """No lead cards means leading/free-form play, so no closed hint set."""
        hand = [_card(Suit.HEARTS, Rank.ACE), _card(Suit.HEARTS, Rank.KING)]
        result = get_legal_play_hints(
            hand, None, Suit.SPADES, Rank.TWO, max_hints=5,
        )
    ...
    def test_following_single_enumerates_all_same_suit_cards(self) -> None:
        ...
        result = get_legal_play_hints(
            hand, lead, Suit.SPADES, Rank.TWO, max_hints=5,
        )
    ...
    def test_following_empty_lead_cards(self) -> None:
        ...
        result = get_legal_play_hints(
            hand, [], Suit.SPADES, Rank.TWO, max_hints=5,
        )
